...
Program::Program(const std::vector<Shader::Ptr> &shaders) : m_handle(0) {
...
bool Program::set_attribute(const Attribute &attr, VertexBuffer::Ptr buffer) {
    ...
    buffer->bind();
    glVertexAttribPointer(location, attr.components(), gl_type(attr.type()),
                          GL_FALSE, attr.stride(),
...
void Program::set_uniform_matrix_4f(const std::string &name,
                                    const float matrix[], bool transpose) {
    int location = glGetUniformLocation(m_handle, name.c_str());
    assert(location != -1);
