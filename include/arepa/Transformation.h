...
void view(const Vec3f &eye,
	  const Vec3f &lookat_point,
	  const Vec3f &up,
	  float mat[16]) {
    ...
    mat[0] = x.x(); mat[4] = x.y(); mat[8]  = x.z(); mat[12] = -eye.dot(x); 
    mat[1] = y.x(); mat[5] = y.y(); mat[9]  = y.z(); mat[13] = -eye.dot(y);
    mat[2] = z.x(); mat[6] = z.y(); mat[10] = z.z(); mat[14] = -eye.dot(z);
    mat[3] =     0; mat[7] =     0; mat[11] =     0; mat[15] =           1;
}

void orthographic(const Vec2f &left_right,
		  const Vec2f &bottom_top,
		  const Vec2f &near_far,
		  float mat[16]) {
    ...
    mat[0] = 2.0f / rl; mat[4] = 0.0f; mat[8] = 0.0f; mat[12] = -(r + l) / rl;
    mat[1] = 0.0f; mat[5] = 2.0f / tb; mat[9]  = 0.0f; mat[13] = -(t + b) / tb;
    mat[2] = 0.0f; mat[6] = 0.0f; mat[10] = -2.0f / fn; mat[14] = -(f + n) / fn;
    mat[3] = 0.0f; mat[7] = 0.0f; mat[11] = 0.0f; mat[15] = 1.0f;
}

void perspective(const Vec2f &left_right,
		 const Vec2f &bottom_top,
		 const Vec2f &near_far,
		 float matrix[16]) {
