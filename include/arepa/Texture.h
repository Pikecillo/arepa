...
class Texture {
  ...
  public:
    ...
    void init(const std::vector<std::uint8_t> &data, std::uint32_t width,
              std::uint32_t height, InternalFormat internal_format,
              Format format, Type type);

    void init(std::uint32_t width, std::uint32_t height,
              InternalFormat internal_format, Format format, Type type);
